...
void searchByName(CustomerAccounts cA[], const string& partialName)
{
    int foundCount = 0;
    for(int i = 0; i < SIZE; i++)
    {
        int matchCount = 0;
        for (int j = 0; j + partialName.length() <= cA[i].name.length(); j++)
        {
            int k;
            for (k = 0; k < partialName.length(); k++)
            {
                if (cA[i].name[j + k] != partialName[k])
                {
                    break;
                }
            }
        ...
        if (matchCount > 0)
        {
            if (foundCount == 0)
            {
                cout << "Accounts matching the partial name '" << partialName << " : " << endl;
            }
            foundCount++;
            cout << "Details of Customer " << i + 1 << " : " << endl;
            cout << "Name of the Customer = " << cA[i].name << endl;
    ...
    if (foundCount == 0)
    {
        cout << "No accounts match the partial name '" << partialName << "'." << endl;
    }
}
...
cout<<"Enter the partial name = ";
cin>>partialName;

searchByName(ca,partialName);

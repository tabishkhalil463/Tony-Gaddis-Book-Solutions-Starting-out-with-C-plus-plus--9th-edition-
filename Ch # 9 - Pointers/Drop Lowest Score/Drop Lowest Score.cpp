...
double averageScoreWithoutLowestScore(double ts[],int s){          //after sorted function excuted then this function is executed
                                                                    // when called and lowest value placed at index 1 by sorted 
                                                                   //function so it skips index 1 and calculate average
double total=0;
    for(int i=1;i<s;i++){
        total+=*(ts+i);
    }
